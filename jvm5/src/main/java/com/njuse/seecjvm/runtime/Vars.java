...
public class Vars {
    ...
    public void setLong(int index, long value) {
        ...
        varSlots[index].setValue((int) (value >> 32));
        varSlots[index + 1].setValue((int) value);
    }
    ...
    public long getLong(int index) {
        if (index < 0 || index + 1 >= maxSize) throw new IndexOutOfBoundsException();
        long high = ((long) varSlots[index].getValue()) << 32;
        long low = ((long) varSlots[index + 1].getValue()) & 0xFFFFFFFFL;
        return high | low;
    }
